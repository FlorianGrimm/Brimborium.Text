...
public interface IBGParserSequenceResult<T1, T2, T3, T4, R> {
    R Select(BGResult<T1> value1, BGResult<T2> value2, BGResult<T3> value3, BGResult<T4> value4, StringRange match);
}

public sealed class BGParserSequence<T1, T2, T3, T4, R> : IBGParser<R> {
    public BGParserSequence(
        IBGParser<T1> parser1,
        IBGParser<T2> parser2,
        IBGParser<T3> parser3,
    ...
    public IBGParser<T2> Parser2 { get; }
    public IBGParser<T3> Parser3 { get; }
    public IBGParser<T4> Parser4 { get; }
    public IBGParserSequenceResult<T1, T2, T3, T4, R> SelectResult { get; }
    //
    public bool Parse(
        BGParserInput input,
        [MaybeNullWhen(false)] out BGResult<R> match,
        ...
        {
            var matchRange = input.Input.Substring(0, next4.Input.Start - input.Input.Start);
            match = new BGResult<R>(matchRange, this.SelectResult.Select(match1, match2, match3, match4, matchRange));
            error = default;
            next = next4;
            return true;
        }
